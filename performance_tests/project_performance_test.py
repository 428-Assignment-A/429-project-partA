...
import string
import time
from api_library import ProjectAPI
from .performance_general import BasePerformanceTester


class ProjectPerformanceTester(BasePerformanceTester):
    ...
    def warm_up(self, count=20):
        ...
        for _ in range(count):
            resp = self.api.create_project(self.generate_random_project())
            if resp.status_code in [200, 201]:
                warmup_ids.append(resp.json()['id'])
        for i in warmup_ids:
            self.api.update_project_put(i, self.generate_random_project())
            self.api.delete_project(i)
        ...
        try:
            for count in range(step, max_objects + step, step):
                ...
                for _ in range(iterations):
                    # Time Create
                    p = self.generate_random_project()
                    t0 = time.perf_counter()
                    r = self.api.create_project(p)
                    ...
                    if r.status_code in [200, 201]:
                        ...
                        # Time Update
                        t0 = time.perf_counter()
                        self.api.update_project_put(
                            nid, self.generate_random_project())
                        t_update += (time.perf_counter() - t0)
                        # Time Delete
                        t0 = time.perf_counter()
                        self.api.delete_project(nid)
...
if __name__ == "__main__":
    tester = ProjectPerformanceTester()
    tester.warm_up()

    # This will generate 100 data points (2000 / 20)
    final_results = tester.run_incremental_suite(max_objects=2000, step=20)

    tester.save_results(final_results, prefix="project_incremental")
    tester.plot_results(final_results, prefix="project_incremental")