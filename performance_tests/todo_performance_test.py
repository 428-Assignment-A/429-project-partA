        ...
        try:
            for count in range(step, max_objects + step, step):
                ...
                for _ in range(iterations):
                    # Time Create
                    p = self.generate_random_todo()
                    t0 = time.perf_counter()
                    r = self.api.create_todo(p)
                    ...
                    if r.status_code in [200, 201]:
                        ...
                        self.api.update_todo_put(
                            nid, self.generate_random_todo())
                        t_update += (time.perf_counter() - t0)
                        # Time Delete
                        t0 = time.perf_counter()
                        self.api.delete_todo(nid)
...
if __name__ == "__main__":
    ...
    # This will generate 25 data points (500 / 20)
    final_results = tester.run_incremental_suite(max_objects=2000, step=20)

    tester.save_results(final_results, prefix="todo_incremental")
    tester.plot_results(final_results, prefix="todo_incremental")
